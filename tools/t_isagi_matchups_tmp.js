...
(async()=>{
  ...
  async function one(opp,i){
    const isagiA=i%2===0;
    const a=isagiA?'isagi':opp, b=isagiA?opp:'isagi';
    ...
    try{
      ...
      const r=await page.evaluate(({dt,maxSim})=>{
        ...
        const i=mains.find(f=>f.key==='isagi');
        const o=mains.find(f=>f.key!=='isagi');
        return {winner,over:!!G.over,t:+(G.t-t0).toFixed(2),isagiHp:i?+i.hp.toFixed(1):null,oppHp:o?+o.hp.toFixed(1):null,
          isagiDmg:i?+(i.dmgDealt||0).toFixed(1):null,oppDmg:o?+(o.dmgDealt||0).toFixed(1):null,
          vision:i?+(i.vision||0).toFixed(1):null};
  ...
  for(const opp of OPP){
    const a=rows.filter(r=>r.opp===opp), wins=a.filter(r=>r.winner==='isagi').length,
      losses=a.filter(r=>r.winner===opp).length, draws=a.length-wins-losses,
      avg=k=>+(a.reduce((sum,r)=>sum+(r[k]||0),0)/a.length).toFixed(1);
    summary.matchups[opp]={matches:a.length,wins,losses,draws,winRate:+(wins/a.length*100).toFixed(1),avgTime:avg('t'),
      avgIsagiHp:avg('isagiHp'),avgOppHp:avg('oppHp'),avgIsagiDamage:avg('isagiDmg'),avgOppDamage:avg('oppDmg'),avgVision:avg('vision'),
      winsAsA:a.filter(r=>r.side==='A'&&r.winner==='isagi').length,winsAsB:a.filter(r=>r.side==='B'&&r.winner==='isagi').length};
  }
  const wins=rows.filter(r=>r.winner==='isagi').length, losses=rows.filter(r=>r.winner===r.opp).length;
  summary.total={matches:rows.length,wins,losses,draws:rows.length-wins-losses,winRate:+(wins/rows.length*100).toFixed(1),
    winsAsA:rows.filter(r=>r.side==='A'&&r.winner==='isagi').length,winsAsB:rows.filter(r=>r.side==='B'&&r.winner==='isagi').length};
  summary.errors=rows.flatMap(r=>r.errors.map(e=>({opp:r.opp,i:r.i,error:e})));
  fs.writeFileSync('isagi_matchup_results_tmp.json',JSON.stringify({summary,rows},null,2));
  console.log('=== ISAGI BENCHMARK SUMMARY ===');
