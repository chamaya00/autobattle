...
Multiverse Battler - a browser-based, single-file 2D canvas autobattler: twelve
fighters across twelve stages, five battle modes, playable as AI-vs-AI or with
a player-controlled fighter (WASD + skill buttons), plus an Adventure mode.
Vietnamese/English UI toggle.

## Stack

Vanilla HTML/CSS/JS. The entire game lives in one file, `index.html` (an IIFE,
no globals), with no build step and no package manager. `play.html` is a
generated, workshop-stripped build of `index.html` (regenerated by
`tools/mk_play.py`); GitHub Pages serves both from `.github/workflows/pages.yml`.
The test suite under `tools/` runs on plain Node with no dependencies to install.

## Commands

- Dev: open `index.html` in a browser directly - no server, no build
- Checks CI runs: `node tools/t_*.js` for each file in `tools/` - the full
  suite is documented below in "8. Kiểm thử", with one file per feature/character

The rest of this document (below) is the project's own working notes for an
agent editing `index.html`, kept as they already were before this repository
was provisioned with the agent factory.
